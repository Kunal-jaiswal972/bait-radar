...
if command -v terraform >/dev/null 2>&1 && $TF output -raw key_vault_name >/dev/null 2>&1; then
  ...
  COSMOS_DB=$($TF output -raw cosmos_database)
  EVENTHUB_NAME=$($TF output -raw eventhub_name)
  GEMINI_MODEL=$($TF output -raw gemini_model)
  VISION_ENDPOINT=$($TF output -raw vision_endpoint)
  LANGUAGE_ENDPOINT=$($TF output -raw language_endpoint)
else
  ...
  VISION_ENDPOINT=$(az cognitiveservices account list -g "$RG" --query "[?kind=='ComputerVision'].properties.endpoint | [0]" -o tsv)
  LANGUAGE_ENDPOINT=$(az cognitiveservices account list -g "$RG" --query "[?kind=='TextAnalytics'].properties.endpoint | [0]" -o tsv)
...
# Read a secret from Key Vault (empty string if it doesn't exist).
kv() { az keyvault secret show --vault-name "$KV" --name "$1" --query value -o tsv 2>/dev/null || true; }
...
cat > "$ROOT/local.settings.json" <<JSON
{
  "IsEncrypted": false,
  "Values": {
...
cat > "$ROOT/transcript-service/local.settings.json" <<'JSON'
{
  "IsEncrypted": false,
  "Values": {
    "AzureWebJobsStorage": "",
